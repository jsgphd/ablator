...
C UNITS: SI, KEV FOR BBT STUFF 
	...
	PARAMETER (N=100,NP1=101,NBIN=45,NSAVE=5)
	...
	DIMENSION ACOND(16), AH2T(10), EOSCOEF(10), SIGMACOEF(7)
...
C  OPAC		OPACITY (ABS. COEFF) IN METERS AT A GIVEN PHOTON ENERGY
...
C SKIP READING ABS.COEFF. FROM THE FILE
	...
	ELSE
C		WRITE(*,*) 'Enter absorbtion coefficient in microns: '
C		READ(*,*) OPACLASER
		READ(9,130) COMMENT
		READ(9,*) OPACLASER
...
C INITIALIZE HYDRO-MOTION VARIABLES
	...
	P0 = F1 + F2 * HG0
      VASAN2 = 0.0
	CALL SOUND(HG0,RHO0,P0,F2,C0,VASAN2)
...
C COMPUTE DEVIATORIC STRESSES AND ENERGIES IF SOLID - NOT USED
...
C COMPUTE NEW SOUND SPEED
		...
		REOS = RHONEW(I)
		PEOS = P(I)
		CALL SOUND(HEOS,REOS,PEOS,F2,CEOS,TM)
...
C USE "SPALL" OR "VAPOR" EOS FOR SPALLED MATERIAL
	ELSE
		...
		IF (HG(I) .GT. 0.0 .AND. HG(I) .LT. 1.0D+15) THEN
			...
			IF (RHONEW(I) .GE. 1D5) THEN
		  WRITE(*,*) 'T=',TNS,' I=',I,' RHONEW=',RHONEW(I),
     &         ' RHO=',RHO(I)
      WRITE(*,*) 'F1=',F1,' F2=',F2,' HG=',HG(I),' P=',P(I)
		END IF
			TNEW(I) = TS
			IF (TNEW(I) .LT. 0.0) THEN
...
C PERFORM ONLY IF SURFACE IS LIQUID
	IF (TNEW(ISURF) .GT. 1.1*TMELT) THEN
...
C CHECK ENERGY BALANCE
	...
	EKIN = 0.0
	DO 92, I=1,N
C TADY BY MELO BYT HSUM = HSUM + H(I) - HSTRESSP(I)
		HSUM = HSUM + H(I)
...
C TIME STEP DIAGNOSTIC SECTION
	IF (DT .LT. 1.0D-17) THEN
		WRITE(*,153) DT,DTHEAT,DTHYDRO,DTVAP,DTTEMP,DTEMPMAX,ITEMPMAX
	END IF
...
C		DTPRESS = 5E-3
	...
	IF (TNS .GT. TPRESS) THEN
		...
		WRITE(14,105) TNS,(TAB,C(K),K=1,N)
...
C CALCULATE RT INSTABILITY PARAMS
		ZMASS1 = 0
		A1 = A(ISPALL+1)
		T1 = 0
		DMELT = 0
C		WRITE(*,*) ITMELT
		DO K=ISPALL+2,ITMELT
			ZMASS1 = ZMASS1 + ZMASS(K)
			A1 = A1 + A(K)
			ZONEDIAM = X(K)-X(K-1)
		...
		IF (DMELT .GT. 0) THEN
			...
			F =  A1 * RHO1
			T1 = T1 / DMELT
C ETA UNITS: KG.M-1.S-1 (=1000 CENTIPOISE)
			ETA = 5.5D-04
			SIGMA = SIGMACALC(T1)
			IF (F .GT. 0 .AND. SIGMA .GT. 0) THEN
				REYNOLDS = DMELT*DSQRT(F*RHO1*DMELT)/ETA
				WEBER = (F*DMELT**2)/SIGMA
			ELSE
				REYNOLDS = 0
				WEBER = 0
			END IF
			WRITE(40,162) TNS,TAB,DMELT,TAB,RHO1,TAB,F,TAB,SIGMA,TAB,T1,TAB,A1,
     &				TAB,REYNOLDS,TAB,WEBER
		END IF
...
105	FORMAT(F12.4,100(A1,E12.5E3))
...
120	FORMAT('Spalled depth (micron) =',F12.6)
...
145	FORMAT(F10.5,101(A1,E12.5E3))
146	FORMAT(F10.5,A1,I5,2(A1,F12.5))
147	FORMAT(F12.7,4(A1,1PE12.5))
148	FORMAT('Vaporized depth (micron) =',F12.6)
149	FORMAT(F12.7,5(A1,1PE12.5))
153	FORMAT('low DT ',6E11.4,I3)
...
C **********************************************************
C DEFINE THE SPECTRUM FUNCTION
	FUNCTION SPECTRUM(JBB)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	
	PARAMETER(NBIN=45)
	DIMENSION BIN(NBIN), BINW(NBIN)
	COMMON/OPACBINS/BIN,BINW
	
	IF (BIN(JBB) .LE. 101.225) THEN
		SPECTRUM = 54.83 * BIN(JBB)
	ELSE
		SPECTRUM = BINW(JBB) * 
     &		(479.64453*(1240/BIN(JBB) -0.01) +
     &		25190.299*DEXP(-(1240/BIN(JBB) - 1.1547276)**2 /0.3190276**2) +
     &		15228.539*DEXP(-(1240/BIN(JBB) - 1.395925)**2 /0.13990381**2) +
     &		9344.5156*DEXP(-(1240/BIN(JBB) - 1.7601666)**2 /0.21062079**2) +
     &		4744.1201*DEXP(-(1240/BIN(JBB) - 2.2334344)**2 /0.47139305**2) +
     &		3530.1509*DEXP(-(1240/BIN(JBB) - 3.1980155)**2 /0.63428837**2) +
     &		5704.8501*DEXP(-(1240/BIN(JBB) - 4.7492857)**2 /1.4205953**2) +
     &		2281.9927*DEXP(-(1240/BIN(JBB) - 7.3327885)**2 /2.3396976**2))
	END IF
...
C UNITS INPUT AS CONVENIENT, BUT CONVERT TO SI IN THIS SUBROUTINE
	SUBROUTINE READMAT
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	DIMENSION ACOND(16),AH2T(10),EOSCOEF(10),SIGMACOEF(7),YIELDCOEF(3)
...
C AND THIRD-ORDER POLYNOMIAL FOR VAPOR
...
C UNITS: P in Pa, T in K
	READ(12,130) COMMENT
	READ(12,*) ABOIL
	READ(12,*) BBOIL
C CONVERT TO SI UNITS (Pa)
C	ABOIL = ABOIL + DLOG10(7.50D+00)
...
C FIRST ZONE GRID SIZE DX METERS
	...
	DX = DX * 1E-9
	RATIO = 1.08
C THESE PARAMETERS GIVE TOTAL X = 56 MICRONS FOR 100 ZONES
...
C DEFINE THE SPECTRUM FUNCTION
C	SPECTRUM(JBB) = BINW(JBB) * 
C &		(20769.104*DEXP(-(1240/BIN(JBB) - 1.1655191)**2 /0.27343741**2) +
C &		15277.489*DEXP(-(1240/BIN(JBB) - 1.3998393)**2 /0.13684392**2) +
C &		7627.8027*DEXP(-(1240/BIN(JBB) - 1.7614986)**2 /0.19257936**2) +
C &		939.38318*DEXP(-(1240/BIN(JBB) - 2.3522129)**2 /0.47139305**2) +
C &		2734.947*DEXP(-(1240/BIN(JBB) - 4.4902906)**2 /2.0971324**2) +
C &		1224.397*DEXP(-(1240/BIN(JBB) - 5.0509443)**2 /0.32013389**2))
...
C UNITS ARE: W/m.K and K IN CURVE FITS
	...
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	DIMENSION AS(0:5), AL(0:5), AV(0:3), AH2T(10)
	...
	CONDL = AL(0) + T*(AL(1) + T*(AL(2) + T*(AL(3) + 
     &		T*(AL(4) + T*AL(5)))))
	CONDV = AV(0) + T*(AV(1) + T*(AV(2) + T*AV(3)))

	T2K = 20000
	IF (T .GT. T2K) THEN
		CONDL = AL(0) + T2K*(AL(1) + T2K*(AL(2) + T2K*(AL(3) + 
     &			T2K*(AL(4) + T2K*AL(5)))))
		CONDV = AV(0) + T2K*(AV(1) + T2K*(AV(2) + T2K*AV(3)))
	END IF

	IF (VOLF .LE. 1.0D-01) THEN
		IF (T .LT. (TMELT-.1)) THEN
			COND = CONDS
C			WRITE(*,*) 'S:COND=',COND,'T=',T
		ELSE IF (T .GT. (TMELT+.1)) THEN
			COND = CONDL
C			WRITE(*,*) 'L:COND=',COND,'T=',T
		ELSE
C TWO PHASE LIQUID/SOLID REQION
			COND = 2. * CONDS * CONDL / (CONDS + CONDL)
C			WRITE(*,*) 'S-L:COND=',COND,'T=',T
		END IF
	ELSE IF (VOLF .GE. 0.99) THEN
		COND = CONDV
C		WRITE(*,*) 'V:COND=',COND,'T=',T
	ELSE
C TWO PHASE LIQUID/VAPOR REGION
		COND = CONDV
C		WRITE(*,*) 'L-V:COND=',COND,'T=',T
	END IF
...
C UNITS ARE: J/kg and K IN CURVE FITS
	...
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	DIMENSION ACOND(16), AS(0:5), EOSCOEF(10)
	DIMENSION YIELDCOEF(3)
	COMMON/MATDAT/TMELT,HMELTI,HMELTT,ACOND,AS,AL0,AL1,AV0,AV1
	COMMON/EOSDAT/RHO0,EOSCOEF,ABOIL,BBOIL,PRNU,YIELDCOEF
...
C ASSUMES IDEAL GAS EOS: P = RHO * RBAR * T
	...
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	DIMENSION ACOND(16),AH2T(10),EOSCOEF(10)
	DIMENSION YIELDCOEF(3)
	COMMON/MATDAT/TMELT,HMELTI,HMELTT,ACOND,AH2T
	COMMON/EOSDAT/RHO0,EOSCOEF,ABOIL,BBOIL,PRNU,YIELDCOEF
...
C F1 = F1(RHO) AND F2 = F2(RHO)
C FOR LINEAR EOS: PRESSURE = F1 + F2 * ENERGY
	SUBROUTINE SOUND(HG,RHO,P,F2,C,TM)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	...
	DET = DF1 + HG*DF2 + P*F2/RHO**2
	IF (DET .GE. 0) THEN
		C = SQRT(DET)
	ELSE
		WRITE(*,*) 'PROBLEM WITH SOUND SPEED: DET=',DET
		C = 1000
	END IF
	
C NEVER
	IF (TM .GE. 2) THEN
		WRITE(40,*) TM*1D+09, I, C, ' C'
		WRITE(40,*) TM*1D+09, I, DF1, ' DF1'
		WRITE(40,*) TM*1D+09, I, DF2, ' DF2'
		WRITE(40,*) TM*1D+09, I, F2, ' F2'
		WRITE(40,*) TM*1D+09, I, HG, ' HG'
		WRITE(40,*) TM*1D+09, I, P, ' P'
		WRITE(40,*) TM*1D+09, I, RHO, ' RHO'
		WRITE(40,*) TM*1D+09, I, DF1 + HG*DF2,' ODMOCNINA1'
		WRITE(40,*) TM*1D+09, I, P*F2/RHO**2, ' ODMOCNINA2'
	END IF
...
C PURE LIQUID
		...
		T = TL
		CAS = 0.0
		CALL SOUND(HG,RHO,PL,F2,CSOUND,CAS)
		QUAL = 0.0
		VOLF = 0.0
	ELSE
...
C NOW COMPUTE SOUND SPEDD
				...
				CAS = 0.0
				CALL SOUND(HGL,RHOL,P,F2,CLIQ,CAS)
...
C UNITS: AL,AV COEFFICIENTS IN J/kg, PRESSURE IN Pa
	FUNCTION QTWO(QUAL)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	DIMENSION ACOND(16),AH2T(10),EOSCOEF(10),YIELDCOEF(3)
...
C OTHERWISE, FIX PRESSURE AT Psat
	IF (QUAL .LE. QMIN) THEN
		...
		P = F1 + F2 * HG
		CAS = 0.0
		CALL SOUND(HG,RHO,P,F2,CSOUND,CAS)
		ALPHA = 0.0D+00
		RHOV = P / (RBAR * T)
	ELSE
...
C **********************************************************
C CALCULATE SURFACE TENSION FROM TEMPERATURE
C USE CUBIC OR EXPONENTIAL FIT TO DATA
	FUNCTION SIGMACALC(T)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	DIMENSION SIGMACOEF(7),YIELDCOEF(3)
...
C FIT PARAMETERS FOR EXPONENTIAL EQUATION
		...
		AEXP = SIGMACOEF(4)
		BEXP = SIGMACOEF(5)
		SIGMACALC = AEXP * 10.**(BEXP * T)
	ELSE
		WRITE(*,*) 'INVALID MATERIAL (SURF TENSION)'
		STOP
	END IF
